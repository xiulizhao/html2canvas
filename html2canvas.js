(function (root, factory) {
  ...
  var loadDependence = function (fncallback) {
    ...
    if (!window.wwload.html2canvas) {
      window.wwload.html2canvas = "wait";
      requirejs.config({
        paths: {
          "canvg": "libs/canvg/canvg.min", // 示例: libs/qrcodejs/qrcode  "libs/html2canvas/dist/rgbcolor.min", "libs/html2canvas/dist/stackblur.min",
          "rgbcolor": "libs/canvg/rgbcolor.min",
          "stackblur-canvas": "libs/canvg/stackblur.min"
        },
        shim: {
          "canvg": {
            deps: ["rgbcolor"]
          ...
          "canvg": {
            deps: ["stackblur-canvas"]
          }
        }
      });
      require(["canvg", "libs/html2canvas/dist/html2canvas.min"], function (canvg, html2canvas) {   //libs/html2canvas/dist/html2canvas
        window.wwload.html2canvas = true;
        window.canvg = canvg;
        window.html2canvas = html2canvas;
        replace();
        fncallback();
      });
    } else if (window.wwload.html2canvas === "wait") {
      setTimeout(function () {
   ...
   */
  function html2_canvas($ele) {
    var config = {};
    config.html2canvasid = $ele.attr("data-html2canvasid");
    $ele.find(".capture").attr("id", config.html2canvasid);
    var html2canvasset = $ele.attr("data-html2canvasset");
    var html2canvasstart = $ele.attr("data--html2canvasstart");
    var targetDom = $ele.find("#" + config.html2canvasid);
    var html2canvasscale = eval($ele.attr("data-html2canvasscale"));

    if (typeof html2canvas !== 'undefined') {
      //以下是对svg的处理
      var nodesToRecover = [];
      var nodesToRemove = [];
      var svgElem = $("#" + config.html2canvasid).find('svg');
      ...
      if (html2canvasset == "true") {
        if (html2canvasstart == "true") {
          ...
          html2canvas(targetDom[0], {
            ...
            setTimeout(function () {
              var imgDate = canvas.toDataURL("image/jpeg", 1);//.replace("image/png", "image/octet-stream");
              var maxwidth = eval($ele.attr("data-maxwidth"));
              var cutdown = $ele.attr("data-cutdown");
              if (cutdown == "true") {
                var base64 = imgDate;
                cutImageBase64(base64, maxwidth, Imgbas);
                //在这里根据回调回来的数据打印
                function Imgbas(base64) {
                  //console.log("压缩后" + base64);
                  $.wwclass.helper.updateProp($ele, "data-x-html2canvas", base64);
                  $ele.attr("data--html2canvasstart", "false");
                  $.wwclass.helper.anijsTrigger($ele, "html2canvas"); //截图输出base64发出的事件
                  // console.log(imgDate);//输出base64格式图片
                }
              } else {
                //调用方法
                //console.log("压缩前" + imgDate);
                $.wwclass.helper.updateProp($ele, "data-x-html2canvas", imgDate);
                $ele.attr("data--html2canvasstart", "false");
                $.wwclass.helper.anijsTrigger($ele, "html2canvas"); //截图输出base64发出的事件
              }
